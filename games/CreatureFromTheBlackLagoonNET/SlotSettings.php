...
class SlotSettings
{
    ...
    public function getNewSpin($game, $lines, $spinWin = 0, $bonusWin = 0, $garantType = 'bet')
    {
        ...
        switch ($lines) {
            case 10: $curField = 10;
                break;
            case 9: case 8: $curField = 9;
                break;
            case 7: case 6: $curField = 7;
                break;
            case 5: case 4: $curField = 5;
                break;
            case 3: case 2: $curField = 3;
                break;
            case 1: $curField = 1;
                break;
        }
    ...
    public function GetRandomScatterPos($rp)
    {
        ...
        if (!is_array($rp)) {
            return rand(2, 5);
        } // Basic fallback if $rp is not an array
        ...
        foreach ($prs as $index => $value) {
            ...
            if (is_array($key) && count($key) > 0) {
                ...
                $safe_key_access = function ($k_arr, $idx) use ($cnt) {
                    if ($idx < 0) {
                        return $k_arr[$cnt + $idx];
                    }
                    if ($idx >= $cnt) {
                        return $k_arr[$idx % $cnt];
                    }
                    return $k_arr[$idx];
                };
                ...
                if ($cnt >= 3) {
                    ...
                    if ($value == 0) {
                        $actual_value_for_rp = 1;
                    } // Use index 1 if value is 0 to avoid -1 issues with simple array access
                    elseif ($value == $cnt - 1) {
                        $actual_value_for_rp = $cnt - 2;
                    } // use index cnt-2 if value is at the end
                    else {
                        $actual_value_for_rp = $value;
                    }
