...
namespace app\games\BiggerBassBlizzard
{

    use app\model\Game;
    use app\model\Collect;
    use app\model\GameSettings;
    use app\model\Loader;
    use app\model\Log;
    use app\model\Spin;
    use app\model\DoMysteryScatter;
    ...
    class Server
    {
        public function get($request)
        {
            try {
                ...
                $user = (object)['id' => $request->userId, 'balance' => $request->balance, 'shop_id' => $request->shop_id];
                $shop = (object)['id' => $request->shop_id];
                $game = (object)['id' => $request->game_id, 'name' => $game, 'shop_id' => $request->shop_id];
                $bank = (object)['shop_id' => $request->shop_id];
                $jpgs = [];
                ...
                if ($action == 'doSpin') {
                    ...
                    $response = Spin::spinResult($user, $game, $bet, $lines, $log, $gameSettings, $index, $counter, $callbackUrl, $pur, $bank, $shop, $jpgs, $request->input('bl'));
                    exit($response);
                }
                ///////////////////////////////////////////
                if ($action == 'doCollect') {
