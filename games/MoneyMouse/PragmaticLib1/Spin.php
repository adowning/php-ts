...
namespace app\games\MoneyMouse\PragmaticLib;

use app\model\Services\Api\Api;
...
class Spin
{
    public static function spinResult($user, $game, $bet, $lines, $log, $gameSettings, $index, $counter, $callbackUrl, $pur, $bank, $shop, $jpgs)
    {
        ...
        $reelSet = 0;
        $pur1 = $pur;
        if ($currentLog && array_key_exists('puri', $currentLog) || $pur1 == '0') {
            $reelSet = 1;
        }
        if ($currentLog && array_key_exists('fs', $currentLog)) {
            $reelSet = 1;
        } // if free spins - then the set of reels is 4th
        ...
        if (SlotArea::getMsCnt($slotArea['SlotArea']) > 5) {
            ...
            $pur1 = '2';
        } elseif ($currentLog && array_key_exists('fs', $currentLog) && $currentLog['fs'] == $currentLog['fsmax']) {
            $slotArea['ScatterCount'] = BuyFreeSpins::addScatters($slotArea['SlotArea'], $gameSettings, 0, 1);
        } elseif ($currentLog && array_key_exists('fs', $currentLog)) {
        ...
        $logAndServer = LogAndServer::getResult(
            $slotArea,
            $index,
            $counter,
            $bet,
